...
public class MatchplayMatchmaker : System.IDisposable
{
    ...
    public async UniTask<MatchmakingResult> Matchmake(UserData data)
    {
        cancelToken = new CancellationTokenSource();

        string queueName = data.UserGamePreferences.ToMultiplayerQueue();
        CreateTicketOptions createTicketOptions = new CreateTicketOptions(queueName);
        Debug.Log(createTicketOptions.QueueName);

        List<Player> players = new List<Player>
        {
            new Player(data.UserID, data.UserGamePreferences)
        };
        ...
        try
        {
            ...
            try
            {
                while (!cancelToken.IsCancellationRequested)
                {
                    ...
                    await UniTask.Delay(TicketCooldown);
                }
            }
